...
<div class= "container mt-5 d-flex justify-content-center ">
	...
	<form action="" method="POST" class="row">
		<div class="mb-3">
			...
			<input type="text" class="form-control" name="fullname" value="<?= isset($_POST['fullname']) ? htmlspecialchars($_POST['fullname']) : $_GET['id'] ?>" required>
		</div>
		...
		<select name="ads" class="form-control">
			...
			<?php var_dump($row['id'])?>
				<option <?= isset($_GET['id_group']) && $_GET['id_group'] == $row['id'] ? 'selected' : $row['id'] ?> value="<?= htmlspecialchars($row['name'].'(' .$row['id'].')') ?>">
